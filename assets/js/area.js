/* ─────────────────────────────────────────────
   ...
   Espera AREA_CONFIG definido no HTML.
...
const _YEARS = [2024, 2025, 2026, 2027];
...
function _baseOpts() {
  ...
  return {
    ...
    plugins: {
      ...
      tooltip: { backgroundColor: c.bg, borderColor: c.border, borderWidth: 1, titleColor: c.title, bodyColor: c.body,
    ...
    scales: {
      ...
      y: { grid:{ color:c.grid }, ticks:{ color:c.tick, font:{ size:11, family:"'Poppins',sans-serif" }, callback: v => v >= 1000 ? (v/1000).toFixed(1)+'k' : v } }
    }
  };
}
  ...
  row.innerHTML = AREA_CONFIG.kpis.map(kpi => {
    ...
    if (m?.prev && m.prev.data?.[kpi.key] != null && raw != null) {
      ...
      if (prev > 0) {
        ...
        const up  = curr >= prev;
        const invertGood = kpi.invertDelta; // true = lower is better (ex: Posição Média)
...
function renderTable(months) {
  ...
  const th = kpis.map(k => `<th${k.currency || !k.pct ? '' : ''} class="num">${k.label}</th>`).join('');
  ...
  const rows = [...months].reverse().map(m => {
    ...
    return `<tr><td style="white-space:nowrap;color:var(--muted);font-weight:600">${_MO[m.mi]} ${m.year}</td>${tds}</tr>`;
  }).join('');
...
(async function init() {
  ...
  const fetches = candidates.map(c =>
    fetch(`/assets/data/${c.year}/${c.mo}/${AREA_CONFIG.file}`)
  ...
  // Link each month to previous for delta
